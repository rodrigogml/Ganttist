...
final class TodoistOAuthController extends Controller
{
    public function redirect(Request $request)
    {
        ...
        $state = Str::random(64);
        DB::table('todoist_oauth_states')->insert(['id' => (string) Str::ulid(), 'user_id' => $request->user()->id, 'state_hash' => hash('sha256', $state), 'expires_at' => now()->addMinutes(10), 'created_at' => now(), 'updated_at' => now()]);
    ...
    public function callback(Request $request)
    {
        ...
        Auth::loginUsingId($state->user_id);
