...
- **Produção:** Apache + PHP-FPM, TLS, scheduler e worker de filas estão ativos no ambiente de pré-produção. Monitoramento externo, alertas, backup/restauração e rollback ainda dependem da infraestrutura.
- **Testes:** a suíte automatizada isolada foi executada com 81 testes PHP/334 asserções e 23 testes frontend; a execução equivalente deve ocorrer em CI antes do release.
...
- Auditoria, fila, histórico de recálculo, retry, SSE e comando `todoist:sync` possuem implementação; worker e reconciliação periódica estão ativos na pré-produção. Métricas e regras de alerta são portáveis, enquanto alertas externos e restauração permanecem configuração de produção.
