...
- **FR-030**: O estado `OPENED` DEVE usar etiqueta e timeblock verdes para comunicar disponibilidade imediata, enquanto `COMPLETED` DEVE usar etiqueta, timeblock e legenda em cinza-escuro, com o título riscado, para diferenciá-lo sem reutilizar o verde nem competir visualmente com tarefas ativas. O filtro por estado calculado DEVE aceitar qualquer combinação de estados por checkboxes e oferecer o agrupador virtual “Desbloqueadas”, que marca ou desmarca simultaneamente `OPENED`, `SCHEDULED` e `LATE`, apresenta estado parcial quando apenas parte do grupo estiver marcada, mantém esses três estados como opções individuais identadas e preserva `BLOCKED` e `COMPLETED` como opções de primeiro nível. O menu DEVE permanecer aberto durante a composição e fechar por Escape ou clique externo.
...
- **SC-011**: Em uma única ação, o usuário consegue exibir todas as tarefas desbloqueadas — abertas, agendadas e atrasadas — e ainda filtrar cada estado separadamente; etiquetas abertas são identificáveis como disponíveis por cor e texto.
