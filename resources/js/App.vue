...
async function saveTask(){if(!activeTask.value)return;const task={...activeTask.value};try{const csrf=document.querySelector<HTMLMetaElement>('meta[name="csrf-token"]')?.content;const response=await fetch(`/api/v1/tasks/${task.id}/dates`,{method:'PUT',headers:{'Content-Type':'application/json',Accept:'application/json',...(csrf?{'X-CSRF-TOKEN':csrf}:{})},body:JSON.stringify({start:task.start,finish:task.finish})});if(!response.ok)throw new Error('Não foi possível salvar as datas no Todoist.');store.updateTask(task);drawer.value=false;toast.value='Alterações salvas no Todoist';setTimeout(()=>toast.value='',3500)}catch(error){toast.value=error instanceof Error?error.message:'Não foi possível salvar as alterações';setTimeout(()=>toast.value='',4500)}}
